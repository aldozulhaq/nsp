...
const GetCustomerNameById = async (_id : any) => {
    const res = await fetch(`/api/costumer/${_id}`, {
...
const GetCustomerArchives = async () => {
    const res = await fetch(`/api/costumer/archives`, {
        headers: { "Authorization" : `Bearer ${localStorage.getItem("token")}`,
        }
    })
    const data = await res.json()
...
const GetCustomerById = async (_id : any) => {
    const res = await fetch(`/api/costumer/getCustId/${_id}`, {
        headers: { "Authorization" : `Bearer ${localStorage.getItem("token")}`,
        }
    })
    const data = await res.json()
...
const EditCustomer = async (_id:any, name:string, desc:string) => {
    const res = await fetch(`/api/costumer/up/${_id}`, {
        method: 'PUT',
        headers: {
            "Content-Type" : 'application/json', 
            "Authorization" : `Bearer ${localStorage.getItem("token")}`,
...
const delCustomer = async (_id:any) => {
    const res = await fetch(`/api/costumer/del/${_id}`, {
        method: 'DELETE',
        headers: { "Authorization" : `Bearer ${localStorage.getItem("token")}`,
        }
    })
...
const softDelCustomer = async (_id:any) => {
    const res = await fetch(`/api/costumer/archive/${_id}`, {
        method: 'PUT',
        headers: { "Authorization" : `Bearer ${localStorage.getItem("token")}`,
        }
    })
...
const restoreCustomer = async (_id:any) => {
    const res = await fetch(`/api/costumer/restore/${_id}`, {
        method: 'PUT',
        headers: { "Authorization" : `Bearer ${localStorage.getItem("token")}`,
        }
    })
